    ...
    const PacketPriority& priority)
{
    ...
    PacketId tmppacketId = packetPtr -> GetPacketId();
    NodeId tmpsorceId = tmppacketId.GetSourceNodeId();

    double txPower = static_cast<double>(channelInfo.txPowerDbm);

    /*char fname[30];
    std::string myTPCS;
    unsigned int myTPC;
    sprintf(fname,"TPC_%d.txt", tmpsorceId);
    std::ifstream inputfile(fname);
    inputfile >> myTPCS;
    myTPC = atoi(myTPCS.c_str());
    ...
    */
    ...
    //if(tmpsorceId == 1){
        //txPower = 7.5;
    //}else if(tmpsorceId == 2){
        //txPower = 10;
    //}

    //書いてみるデフォで32
    //std::cout << "txpower" << tmpsorceId << " = " << txPower << endl;
    /*if(tmpsorceId == 10){
        std::cout << "TxP2 = " << txPower << endl;
    }*/
