    ...
    const SimTime& txopDuration)
{
    ...
    //試し
    //std::cout << "SetedcaParameter"<< std::endl;
    //ここは通ってない

    //dot11のmacポインタをdot11のSetEdcaParameterに渡している
    channelEntities.at(channelNumberId).macPtr->SetEdcaParameter(
            accessCategoryIndex,    //これ辿ってみようかな
...
void WaveMac::InsertPacektIntoCchOrSchQueueWhichSupportsChannelIdOf(
    ...
    const NetworkAddress& nextHopAddress,
    const PacketPriority priority,
    //設定にあるプライオリティを辿ったらここに送られている(ネットワーク層のsendWsmに渡された後)
    const EtherTypeField& etherType,
    const DatarateBitsPerSec& theDatarateBitsPerSec,
    const double txPowerDbm,
    EnqueueResultType& enqueueResult,
    unique_ptr<Packet>& packetToDropPtr)
{
    ...
    //RSSI書いてみた-になる...
    /*const PacketId tpacketId = packetPtr->GetPacketId();
    const NodeId SourceId = tpacketId.GetSourceNodeId();

    double rssi = channelEntities.at(channelNumberId).macPtr->GetRssiOfLastFrameDbm();
    std::cout << "rssi = " << (int)rssi << endl;*/

    

    /*char fname[30];
    sprintf(fname,"tmp_%d.txt",basicSafetyMessageInfo.MyNodeId);
    std::ofstream outputfile(fname);
    outputfile << (int)basicSafetyMessageInfo.priority << std::endl;
    outputfile.close();*/

    channelEntities.at(channelNumberId).outputQueuePtr->
        //その後ここに送られてるけどわからん（its_queues.hにある）
...
void WaveMac::PhyEntity::ResumeTxRxOperationAndScheduleEndEventIfNecessary()
{
    ...
    //バックオフ実行
    //この辺で速度とかpriorityとか取れないかなあ
    //ID取れた
    unsigned int myId = waveMac.simEngineInterfacePtr -> GetNodeId();
    //std::cout << "myId = " << (int)myPriority << endl;


    //読み取り
    /*char fname[30];
    std::string myPriorityS;
    unsigned int myPriority;
    sprintf(fname,"tmpPri_%d.txt",myId);
    ...
    //if(!inputfile){
        ...
        //std::cin.get();
    //}
    inputfile >> myPriorityS;
    myPriority = atoi(myPriorityS.c_str());

    inputfile.close();*/

    //成功
    //std::cout << myPriority << endl;
